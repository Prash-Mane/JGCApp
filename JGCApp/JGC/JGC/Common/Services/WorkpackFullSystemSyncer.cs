...
{
    public class WorkpackFullSystemSyncer : IWorkpackFullSystemSyncer
    {
        ...
        #endregion
        public WorkpackFullSystemSyncer
            (
            ...
            IRepository<T_ITRInsulationDetails> _InsulationDetailsRepository,
             ...
             IRepository<T_ITR_8300_003A_Illumin> _ITR_8300_003A_IlluminRepositor,
        ...
        public async Task<bool> uploadChanges()
        {
            ...
            foreach (T_TAG tag in allTags)
            {
                ...
                foreach (var checkSheet in CehckSheetPerTAgs)
                {
                    ...
                    //Select * FROM T_TAG_SHEET_ANSWER WHERE tagName = 'AC34' AND checksheetname = 'E01B'  AND jobpack = "" issynced=0
                                ...
                                foreach (SignOff signature in _SignOffsHeader)
                                {
                                    ...
                                    switch (signature.count)
                                    {
                                        case 1:
                                            ...
                                            uploadCmsDto.REMARKS = unsyncedAnswers.Any() ? unsyncedAnswers.FirstOrDefault().checkValue : "";
                                            break;
                                        case 2:
        ...
        private async Task<bool> pullDownWorkpacks(string WorkpackName)
        {
            try
            {
                ...
                {
                    tagToSheetList = WorkpacksList.tagNameToSheetNameMap;
                    foreach (var tag in tagToSheetList)
                    {
                        foreach (var SheetName in tag.Value)
                        {
                            ...
                            bool CCSITR = WorkpacksList.checkSheets.Where(x => x.name == SheetName).FirstOrDefault().ccsItr;                          
                ...
                if (WorkpacksList.tags != null)
                {
                    ...
                    await _TAGRepository.InsertAllAsync(WorkpacksList.tags);
                }
                if (WorkpacksList.checkSheets != null)
                {
                    WorkpacksList.checkSheets.ForEach(i => i.ProjectName = Settings.ProjectName);
                    await _CHECKSHEETRepository.InsertAllAsync(WorkpacksList.checkSheets);
                }

                List<ITRs> ITRData = WorkpacksList.checkSheets.Where(i => i.ITRData != null).Select(i => i.ITRData).ToList();
        ...
        private void pullDownTagHeaders()
        {
            try
            {
                // tag headers data not found yet
                //foreach (string Tag in tagToSheetList.Keys)
                //{
                //    var JsonString = ModsTools.WebServiceGet(ApiUrls.getTagHeaders(Settings.ModelName, Settings.ProjectName, Settings.CurrentDb, Tag, "test"), Settings.AccessToken);
                //    var SystemsList = JsonConvert.DeserializeObject<List<T_Tag_headers>>(JsonString);

                //    if (JsonString != "{}" || !string.IsNullOrWhiteSpace(JsonString))
                //    {
                //        _Tag_headersRepository.InsertOrReplaceAsync(SystemsList);
                //    }
            ...
            {
                _userDialogs.AlertAsync("error  tag download", "", "OK");
            }
            //  string Result = ModsTools.WebServiceGet(ApiUrls.getTagHeaders(Settings.ModelName, Settings.ProjectName, "workpack", "WorkpackName", "dontmatter", Settings.CurrentDb), Settings.AccessToken);
            // var SystedmsList = JsonConvert.DeserializeObject<T_WorkPack>(Result);
        }
        private async Task<bool> pullDownSheetQuestions()
        {
            ...
            foreach (string sheetname in sheetNames)
            {
                try
                {
                    ...
                    {
                        //var InsertQuetionsList = Quetions.Select(x => new T_CHECKSHEET_QUESTION()
                        //{
                        //    id = x.id,
                        //    section = x.section,
                        //    answerOptional = x.answerOptional,
                        ...
                        //  Quetions.ForEach(x => { x.CheckSheet = sheetname; x.ProjectName = Settings.ProjectName; });
        ...
        {
            foreach (var TagName in tagToSheetList)
            {
                foreach (string sheetname in TagName.Value)
                {
                    try
                    {
                        ...
                        foreach (CheckSheetAnswerModel checkSheetAnswer in CheckSheetAnswers)
                        {
                            ...
                            {
                                //var SignOffList = checkSheetAnswer.SignOffHeaders.Select(x => new T_SignOffHeader()
                                //{
                                //    Count = x.Count,
                                //    FullName = x.FullName,
                                //    Rejected = x.Rejected,
                                //    RejectedReason = x.RejectedReason,
                                //    SignDate = x.SignDate,
                                //    SignOffChecksheet = checkSheetAnswer.CheckSheetName,
                                //    SignOffTag = checkSheetAnswer.TagName,
                                //    Title = x.Title,
                                //    ProjectName = Settings.ProjectName
                                //});
                            ...
                            {
                                //var dbdata = checkSheetAnswer.answers.Select(x => new T_TAG_SHEET_ANSWER()
                                //{
                                //    tagName = checkSheetAnswer.TagName,
                                //    ccmsHeaderId = checkSheetAnswer.CcmsHeaderId,
                                ...
                                //{
                                //    await _TAG_SHEET_ANSWERRepository.InsertOrReplaceAsync(dbdata);
                                //}

                                checkSheetAnswer.answers.ForEach(x=> { x.tagName = checkSheetAnswer.TagName;x.ccmsHeaderId = checkSheetAnswer.CcmsHeaderId; x.checkSheetName = checkSheetAnswer.CheckSheetName; x.jobPack = checkSheetAnswer.JobPack; x.IsSynced = true; x.ProjectName = Settings.ProjectName; });
        ...
        private async Task<bool> Post_8121_004A_ITRsData(string Tag, string Checksheet)
        {
            try
            {
                ...
                itrDataObj.CommonHeaderFooter = itrDataObj.CommonHeaderFooter = await GetCommonHeaderFooterDataforPost(Tag, Checksheet);
                if (itrDataObj.CommonHeaderFooter != null)
                {
        ...
        private async Task<bool> Post_8170_002A_ITRsData(string Tag, string Checksheet)
        {
            ...
            try
            {
                ...
                if (itrDataObj.CommonHeaderFooter != null)
                {
                    string sqlQuery = " SELECT * FROM T_ITR_8170_002A_IndifictionLamp WHERE ITRNumber='" + itrDataObj.CommonHeaderFooter.ITRNumber + "' AND CommonRowID ='" + itrDataObj.CommonHeaderFooter.ROWID + "' AND ModelName = '" + Settings.ModelName + "' AND CCMS_HEADERID = " + itrDataObj.CommonHeaderFooter.ID;
                    ...
                    string VoltRead = "SELECT * FROM T_ITRRecords_8170_002A_Voltage_Reading WHERE ITRCommonID = '" + itrDataObj.CommonHeaderFooter.ID + "' AND CommonRowID = '" + itrDataObj.CommonHeaderFooter.ROWID + "' AND ModelName = '" + Settings.ModelName + "' AND CCMS_HEADERID = " + itrDataObj.CommonHeaderFooter.ID;
