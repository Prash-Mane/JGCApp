...
{
    public class DrainRecordViewModel : BaseViewModel
    {
        ...
        private readonly IRepository<T_AdminDrainRecordAcceptedBy> _AdminDrainRecordAcceptedByRepository; 
        ...
        public DrainRecordViewModel(
            ...
            IRepository<T_AdminDrainRecordAcceptedBy> _AdminDrainRecordAcceptedByRepository,
        ...
        IRepository<T_UserProject> _userProjectRepository) : base(_navigationService, _httpHelper, _checkValidLogin)
        {
            ...
            this._AdminDrainRecordAcceptedByRepository = _AdminDrainRecordAcceptedByRepository;
        ...
        private async Task LoadCertificationTabsAsync()
        {
            try
            {
               ...
               // var CurrentUserProject = await _userProjectRepository.GetAsync(x => x.Project_ID == Settings.ProjectID);
                var tsk1 = await GenerateContentTableAsync(Settings.ProjectID, CurrentPageHelper.ETestPackage.ID, "T_AdminDrainRecordContent", "T_DrainRecordContent", CurrentUserProject);
                var tsk2 = await GenerateAcceptedByTableAsync(Settings.ProjectID, CurrentPageHelper.ETestPackage.ID, "T_AdminDrainRecordAcceptedBy", "T_DrainRecordAcceptedBy", CurrentUserProject);
        ...
        private async Task<List<RecordConfirmation>> GenerateContentTableAsync(int projectID, int etestPackageID, string adminDatabaseTable, string databaseTable, T_UserProject userProject)
        {
            ...
            try
            {
                ...
                var reuslt = await _adminDrainRecordContentRepository.QueryAsync<RecordConfirmation>(SQL);
        ...
        public async Task<List<RecordAcceptedBy>> GenerateAcceptedByTableAsync(int projectID, int etestPackageID, string adminDatabaseTable, string databaseTable, T_UserProject userProject)
        {
            ...
            var result = await _drainRecordAcceptedByRepository.QueryAsync<RecordAcceptedBy>(SQL);
        ...
        public async void gvDrainRecordContent_CellContentClick(string param, RecordConfirmation SelectedRecord)
        {
            ...
            if (SelectedConfirmationSource != null || btnNA == true) //Status Image
            {
               ...
               Boolean live = SelectedConfirmationSource.Live;
                ...
                if (await CanSignOffCertificiationSignature("T_AdminDrainRecordContent", "T_DrainRecordContent", adminID, signed, live))
                {
                    T_DrainRecordContent signature = new T_DrainRecordContent()
                    {
                        AdminID = adminID,
                        Signed = !signed,
                    ...
                    if (await SaveNew(Settings.ProjectID, CurrentPageHelper.ETestPackage.ID, "T_DrainRecordContent", signature))
                    {
                        UpdateETestPackageStatus(Settings.ProjectID, CurrentPageHelper.ETestPackage.ID);
        ...
        public async void gvDrainRecordAcceptedBy_CellContentClick(string param, RecordAcceptedBy SelectedRecord)
        {
            ...
            if (SelectedrecordAcceptedBySource != null) //Status Image
            {
                ...
                if (await CanSignOffAcceptedBySignature("T_AdminDrainRecordAcceptedBy", "T_DrainRecordAcceptedBy", RecordConfirmationSource, adminID, signed, live, orderNo))
                {
                    ...
                    if (await SaveNew(Settings.ProjectID, CurrentPageHelper.ETestPackage.ID, "T_DrainRecordAcceptedBy", signature))
                    {
                        if (!signed)
                        {
                            ...
                            string SQL = "SELECT * FROM [T_AdminDrainRecordAcceptedBy] WHERE [ProjectID] = '" + Settings.ProjectID + "' AND [ID] = '" + CurrentPageHelper.ETestPackage.ID + "'";
                            var result = await _AdminDrainRecordAcceptedByRepository.QueryAsync<T_AdminDrainRecordAcceptedBy>(SQL);
        ...
        public async Task<bool> SaveNewAsync(int projectID, int etestPackageID, T_ControlLogSignature controlLogModel)
        {
...

            {
                ...
                string SQL = " UPDATE [T_ControlLogSignature] SET[Reject] = 0, [Signed] = " +Convert.ToInt32(controlLogModel.Signed)+ ",[SignedByUserID] = '"
                    + controlLogModel.SignedByUserID + "' , [SignedBy] = '" + controlLogModel.SignedBy + "',[SignedOn] = '" + controlLogModel.SignedOn.ToString("dd-MMM-yyyy")
                    + "', [Live] = " + false + ", [Updated] = 1 WHERE[ProjectID] = " + projectID + " AND[ETestPackageID] = " + etestPackageID
        ...
        private async Task<Boolean> CanSignOffCertificiationSignature(string adminDatabaseTable, string databaseTable, int adminID, Boolean signed, Boolean live)
        {
            ...
            var result = await _adminDrainRecordContentRepository.QueryAsync<T_AdminDrainRecordContent>(SQL);
        ...
        public async Task<Boolean> SaveNew(int projectID, int etestPackageID, string databaseTable, T_DrainRecordContent signature)
        {
            if (databaseTable == "T_DrainRecordContent")
            {
                ...
                var sqlreturn = await _DrainRecordContentRepository.QueryAsync(SQL);
                ...
                if (sqlreturn.Any())
                {
                    ...
                    try
                    {
                        ...
                        var data = _DrainRecordContentRepository.QueryAsync(sql);
                        return returnvalue;
                    }
                    catch (Exception Ex)
                    {
            ...
            else 
            {
                ...
                string SQL = "SELECT *  FROM [" + databaseTable + "] WHERE [ProjectID] = '" + projectID + "' AND [ETestPackageID] = '" + etestPackageID + "' AND [AdminID] = '" + signature.AdminID + "'";
                var sqlreturn = await _drainRecordAcceptedByRepository.QueryAsync(SQL);
                ...
                if (sqlreturn.Any())
                {
                    ...
                    try
                    {
                        ...
                        var data = _drainRecordAcceptedByRepository.QueryAsync(sql);
                        return returnvalue;
                    }
                    catch (Exception Ex)
                    {
